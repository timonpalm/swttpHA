...
public class CannonGame extends Game implements Serializable{
	...
	public void setBoard(String state) {
		...
		//TODO: implement
		String [] rowsOfPanel = state.split("/");
		
		int rowCounter = 0;
		for(String row : rowsOfPanel){
			// if no figure is in row
			if(row == ""){
				// fill row with null
				for(int i=0; i<10; i++){
					gameBoard[rowCounter][i] = null;
				}
			}
			// if something is in row
			else{
				int positionCount = 0;	// count the position in row
				// go through all chars in row 
				for(int n=0; n<row.length(); n++){
					Character sign = row.charAt(n);
					// if sign is a number
					if(Character.isDigit(sign) == true){
						int position = Integer.parseInt( String.valueOf(sign) );	// parse to int
						// with the row with position numbers
						for(int k=positionCount; k<positionCount + position; k++){
							gameBoard[rowCounter][k] = null;
						}
						positionCount += position;
					}
					// save the sign in matrix
					else{
						gameBoard[rowCounter][positionCount] = sign;
					}
					positionCount++;
				}
			}
	...
	public String getBoard() {
		...
		String board = "";
		Integer counter; //counter for empty spaces
		for (int i = 0; i<10; i++){
			counter = 0;
			for (int j = 0; j<10 ; j++){
				//if is not empty
				if (gameBoard[i][j] != null){
					if (counter != 0) board += counter.toString();
					board += gameBoard[i][j];
					counter = 0;
				}
				//if is empty count empty spaces
				else{
					counter++;
				}
			}
